...
ScrollIndicator = (function () {
    ...
    function update(val) {
        ...
        for (var i = 0; i < sections.length; i++) {
            var activeTitle = sections[i].getAttribute('id'),
                activeSection = scrollIndicator.querySelector('[id=' + activeTitle + ']');
    ...
    function init() {
        ...
        for (var i = 0; i < sections.length; i++) {
            ...
            li.setAttribute('id', section.getAttribute('id'));
            a.innerHTML = count;
            a.id = id;
...
ScrollIt = function (target, time) {
    var targetId = target.id,
        end = document.getElementById(targetId).offsetTop - 60,
    ...
    cambiar_pos_directamente(targetId);
}